...
class DataProcessor:
    ...
    def is_degenerate(self , col: str) -> bool:
        ...
        if self.data.select(isnan(col).cast('integer').alias(col)).agg({col:'mean'}).first()[0] >= self.config.get('max_null_share', 0.999):
            return True
    ...
    def is_categorical(self, col: str) -> bool:
        ...
        if isinstance(self.data.schema[col].dataType, (DateType, TimestampType)):
            return False
        if not isinstance(self.data.schema[col].dataType, (IntegerType, LongType, ShortType, ByteType, FloatType, DoubleType, DecimalType)):
            if col.endswith(self.config.get('numerical_suffixes', ())):
...
class PanelDataProcessor(DataProcessor):
    ...
    def check_panel_consistency(self) -> None:
        ...
        subset = self.data.select([self.config['individual_identifier'], self.config['time_identifier']])
    ...
    def process_all_columns(self, indiv_id: str = None) -> pyspark.sql.DataFrame:
        ...
        for col_name in self.data.columns:
            self.data = self.process_single_columns(colname = col_name)
        return self.data
    ...
    def flag_validation_individuals(self, individual_identifier: 'str', validation_share: float = 0.25) -> pyspark.sql.DataFrame:
        ...
        self.data = self.data.withColumn('val_flagged', self.data[self.config['individual_identifier']].isin(unique_list))
        return self.data
    ...
    def build_reserved_cols(self):
        ...
        max_test_intervals = int((self.data.select('_period').distinct().count() - 1) / 2)
        if self.config.get('test_intervals', 0) > max_test_intervals:
            self.config['test_intervals'] = max_test_intervals

        self.data = self.data.withColumn('_test', (self.data['_period'] + self.config.get('test_intervals', 0)) >= max_val)
        ...
        self.data = self.data.withColumn("prev_period", lag(self.data['_period']).over(period_window))
        self.data = self.data.withColumn("gaps", self.data.prev_period < (self.data['_period'] - 1)).fillna(False)
        self.data = self.data.withColumn("gaps", self.data.gaps.cast('int'))
        self.data = self.data.withColumn("_spell", F.sum(self.data.gaps).over(period_window))

        duration_window = Window.partitionBy([self.config['individual_identifier'], '_spell']).orderBy("_period")
        self.data = self.data.withColumn('_duration', F.count(self.data['_spell']).over(duration_window))
        max_duration_window = Window.partitionBy([self.config['individual_identifier'], '_spell']).orderBy(col("_period").desc())
        self.data = self.data.withColumn('max_duration', F.max(self.data['_duration']).over(max_duration_window))
        self.data = self.data.withColumn('_duration', self.data.max_duration - self.data['_duration'])
        self.data = self.data.withColumn('_event_observed', self.data['_duration'] < self.data['_maximum_lead'])
